...
export default function AboutPage() {
  ...
  return (
    <>
      ...
      {/* ── Hero ───────────────────────────────────────────── */}
      <section style={{ position: 'relative', height: isMobile ? '80vh' : '92vh', overflow: 'hidden', minHeight: '500px' }}>
        <img
        ...
        />
        <div style={{ position: 'absolute', inset: 0, background: isMobile ? 'rgba(26,24,20,0.55)' : 'linear-gradient(100deg, rgba(26,24,20,0.78) 38%, rgba(26,24,20,0.05) 72%)' }} />

        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: `0 ${px} ${isMobile ? '48px' : '80px'}`, maxWidth: isMobile ? '100%' : '620px' }}>
          <span className="eyebrow eyebrow-light reveal" style={{ marginBottom: '20px' }}>About QOM</span>
          <h1 className="reveal d1" style={{
            fontFamily: "'Cormorant Garamond', serif",
            fontSize: isMobile ? '52px' : 'clamp(52px, 7.5vw, 100px)',
        ...
        {/* Year founded chip — hide on mobile */}
        {!isMobile && (
          <div style={{
            position: 'absolute', bottom: '80px', right: px,
      ...
      {/* ── Brand statement ─────────────────────────────────── */}
      <section style={{ background: '#1A1814', padding: `${isMobile ? '56px' : '80px'} ${px}` }}>
        <div style={{ maxWidth: '1440px', margin: '0 auto' }}>
          <p className="reveal" style={{
            fontFamily: "'Cormorant Garamond', serif",
            fontSize: isMobile ? '22px' : 'clamp(26px, 3.5vw, 44px)',
            fontWeight: 300, fontStyle: 'italic',
            color: '#F5F0E8', lineHeight: 1.25,
            maxWidth: '820px',
            borderLeft: '2px solid #8B6F47', paddingLeft: isMobile ? '20px' : '36px',
          }}>
      ...
      <section style={{ background: '#F5F0E8' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr' }}>
          {/* Left: text */}
          <div className="reveal-left" style={{ padding: isMobile ? '56px 20px' : isTablet ? '64px 40px' : '96px 80px 96px 64px', display: 'flex', flexDirection: 'column', gap: '32px', justifyContent: 'center' }}>
            <div>
              <span className="eyebrow" style={{ marginBottom: '16px' }}>Our Story</span>
              <h2 style={{
                fontFamily: "'Cormorant Garamond', serif",
                fontSize: isMobile ? '36px' : 'clamp(34px, 4vw, 52px)',
                fontWeight: 300, fontStyle: 'italic',
          ...
          <div className="img-zoom reveal-right" style={{ overflow: 'hidden', minHeight: isMobile ? '80vw' : '640px', position: 'relative', background: '#E0D9CF' }}>
            <img
      ...
      <section style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? '1fr 1fr' : '1fr 1fr 1fr', gap: '3px', background: '#FAFAF8' }}>
        {[
        ...
        ].slice(0, isMobile ? 2 : 3).map(({ src, alt, caption }, i) => (
          <div key={i} className={`img-zoom reveal d${i + 1}`} style={{ position: 'relative', overflow: 'hidden', background: '#D6CFC3', aspectRatio: isMobile ? '4/3' : '3/4' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '55% 1fr', minHeight: isMobile ? 'auto' : '520px', background: '#FAFAF8' }}>
        {/* Factory image */}
        <div className="img-zoom reveal-left" style={{ position: 'relative', overflow: 'hidden', background: '#C8C0B4', minHeight: isMobile ? '60vw' : 'auto' }}>
        ...
        <div className="reveal-right" style={{ display: 'flex', flexDirection: 'column', background: '#FFFFFF' }}>
          {[
            ...
            <div key={i} style={{
              flex: 1, padding: isMobile ? '28px 20px' : '44px 56px',
              borderBottom: i < 2 ? '1px solid rgba(26,24,20,0.07)' : 'none',
              display: 'flex', flexDirection: 'column', justifyContent: 'center',
            }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: isMobile ? '40px' : '56px', fontWeight: 300, fontStyle: 'italic', color: '#1A1814', lineHeight: 0.9 }}>{stat.n}</div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', minHeight: isMobile ? 'auto' : '580px', background: '#FAFAF8' }}>
        {/* Dark text side */}
        <div className="reveal-left" style={{ background: '#1A1814', padding: isMobile ? '56px 20px' : isTablet ? '64px 40px' : '80px 72px 80px 64px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '28px' }}>
          <span className="eyebrow eyebrow-light" style={{ marginBottom: '4px' }}>Why Group Buying</span>
          <h2 style={{
            fontFamily: "'Cormorant Garamond', serif",
            fontSize: isMobile ? '32px' : 'clamp(32px, 3.5vw, 48px)',
            fontWeight: 300, fontStyle: 'italic',
        ...
        {/* Image side */}
        <div className="img-zoom reveal-right" style={{ overflow: 'hidden', position: 'relative', background: '#E8E4DE', minHeight: isMobile ? '80vw' : 'auto' }}>
          <img
      ...
      <section style={{ position: 'relative', minHeight: isMobile ? '40vh' : '56vh', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: `40px ${px}` }}>
          <p className="reveal" style={{
            fontFamily: "'Cormorant Garamond', serif",
            fontSize: isMobile ? '22px' : 'clamp(26px, 4vw, 52px)',
            fontWeight: 300, fontStyle: 'italic',
      ...
      <section style={{ background: '#1A1814', padding: `${isMobile ? '64px' : '88px'} 0` }}>
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: `0 ${px} 48px` }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? '1fr 1fr' : 'repeat(3, 1fr)', gap: '2px', background: 'rgba(196,168,130,0.06)' }}>
          {[
            {
              icon: '◎',
              title: 'Zero Surplus',
            ...
            <div key={pillar.title} className={`reveal d${i + 1}`} style={{ background: '#1A1814', padding: isMobile ? '40px 20px' : '56px 48px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
      ...
      {/* ── Final CTA ──────────────────────────────────────────── */}
      <section style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', minHeight: isMobile ? 'auto' : '480px', background: '#FAFAF8' }}>
        <div className="img-zoom reveal-left" style={{ overflow: 'hidden', background: '#E0D8CE', minHeight: isMobile ? '60vw' : 'auto' }}>
          <img
        ...
        <div className="reveal-right" style={{ background: '#F5F0E8', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center', padding: isMobile ? '48px 20px' : isTablet ? '64px 40px' : '80px 64px' }}>
